...
const ErrorsOverviewPlot: React.FC = () => {
  ...
  return (
    <div data-testid="exceptions-plot" className="flex font-body items-center justify-center w-full h-[36rem]">
      {status === "pending" && (
      ...
      {status === "success" && plot === null && (
        <p data-testid="exceptions-plot-no-data" className="text-lg font-display text-center p-4">No Data</p>
      )}
      {status === "success" && plot !== null && plot !== undefined && (
        <div data-testid="exceptions-plot-data" className="w-full h-full">
        <ResponsiveLine
          data={plot}
          curve="monotoneX"
          theme={chartTheme}
