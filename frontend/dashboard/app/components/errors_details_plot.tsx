...
const ErrorsDetailsPlot: React.FC<ErrorsDetailsPlotProps> = ({
  ...
  return (
    <div data-testid="exception-detail-plot" className="flex font-body items-center justify-center w-full md:w-1/2 h-[32rem]">
      ...
      {effectiveStatus === "success" && plot === null && (
        <p data-testid="exception-detail-plot-no-data" className="text-lg font-display text-center p-4">No Data</p>
      )}
      {effectiveStatus === "success" && plot !== null && plot !== undefined && (
        <div data-testid="exception-detail-plot-data" className="w-full h-full">
        <ResponsiveLine
          data={plot}
          curve="monotoneX"
          theme={chartTheme}
