...
const ANDROID_BUG_REPORT_DESCRIPTION = "e2e-android-bug-report";

test.describe("bug reports", () => {
  test("overview lists the android bug report", async ({ page }) => {
    await page.goto(`/${teamId}/bug_reports?a=${appId}`);

    const row = page
      .getByTestId("bug-report-row")
      .filter({ hasText: ANDROID_BUG_REPORT_DESCRIPTION });
  ...
  test("clicking the row opens the bug report detail page", async ({
    ...
    await expect(page.getByTestId("bug-report-detail-description")).toHaveText(
      ANDROID_BUG_REPORT_DESCRIPTION,
    );
